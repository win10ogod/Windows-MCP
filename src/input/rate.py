...
class RateConfig:
    ...
    mouse_max_delta: int = 60  # px per step
    mouse_smooth: float = 0.0  # 0..1, 0=no smoothing
...
class RateLimiter:
    ...
    def filter_target(self, cur: tuple[int, int], target: tuple[int, int]) -> tuple[int, int]:
        ...
        if md > 0:
            if abs(dx) > md:
                dx = md if dx > 0 else -md
            if abs(dy) > md:
                dy = md if dy > 0 else -md
    ...
    # Utility for synchronous throttling
